...
public partial class searchcaritem : System.Web.UI.Page
{
    ...
    private void BindRepeater()
    {
        ...
        using (SqlConnection con = new SqlConnection(constr))
        {
            ...
            using (SqlCommand cmd = new SqlCommand("select  TOP 100 PERCENT  name,type, ac_non_ac , plate_no , seat , cost_per_day   from vehicle where route_id in ( select route_id from route where cityname='" + Session["f"].ToString() + "' and destination ='" + Session["t"].ToString() + "' or destination='" + Session["f"].ToString() + "' and cityname ='" + Session["t"].ToString() + "' and  book_end  < '" + dtime + "'  ) and type='micro' ", con))
            {
